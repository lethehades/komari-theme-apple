...
function createSkippedNode(
  node: NodeData,
  skipReason: SkipReason
): SkippedRemainingValueNode {
  return {
    uuid: node.uuid,
    name: node.name,
    skipReason,
...
function createRemainingValueNode(
  ...
  const monthlyCostOriginal =
    node.billing_cycle > 0 ? (node.price * 30) / node.billing_cycle : 0;
...
export function buildRemainingValueSnapshot(
  ...
  active.sort((left, right) => right.remainingValueOriginal - left.remainingValueOriginal);
...
export function formatRemainingDays(remainingMs: number | null, isLongTerm: boolean) {
  if (remainingMs === null) return "永久";
  if (isLongTerm) return "长期";
